...
public class CustomerAuthService(ApplicationDataContext dbContext,
...
ISmsService smsService) : ICustomerAuthService
{
    public async Task<Result<LoginCustomerResponseModel>> LoginAsync(IndividualLoginRequestModel requestModel)
    {
        ...
        if (customer == null || customer.User == null || customer.User.IsDeleted)
        ...
        {
            responseModel.IsPhoneNumberConfirmed = user.PhoneNumberConfirmed;

            var storedOtpCode = await dbContext.UserOtps
                                           .Include(x => x.User)
    ...
    public async Task<Result<MessageResponseModel>> Login2FaAsync(Verify2FaCustomerRequestModel requestModel)
    {
        ...
        if (customer == null || customer.User == null || customer.User.IsDeleted || customer.User.TotpTokenEnd < DateTimeOffset.UtcNow)
    ...
    public async Task<Result<MessageResponseModel>> RefreshTokenAsync()
    {
        ...
        try
        {
            ...
            return Result<MessageResponseModel>.Success(new MessageResponseModel("Refresh Token Generated Successfully"), statusCode: HttpStatusCode.NoContent);
        }
        catch (Exception)
        {
